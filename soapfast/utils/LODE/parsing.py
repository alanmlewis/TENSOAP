...
def add_command_line_arguments_PS(parsetext):
    ...
    parser.add_argument("-d", "--dummy",       type=int,default=0,                              help="Add dummy atom at the boundary of each environment, at -x (1), -y (2), or -z (3)")
...
def set_variable_values_PS(args):
    ...
    return [nmax,lmax,rc,sg,cen,spec,cw,lam,periodic,ncut,sparsefile,frames,subset,sparse_options,outfile,args.initial,atomic,all_radial,not args.uselist,xyz_slice,args.imag,args.nonorm,ele,sew,srad,rad,leb,ave,args.vol_frac,args.dummy]
